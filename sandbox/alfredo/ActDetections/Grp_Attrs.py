...
class Grp_Attrs(object):
    def __init__(self, dfIndAttrs, bSaveFrms, imgLims):
        ...
        self.dfGrp_Attrs = [];
        self.grp_Detector = Grp_Detector(dfIndAttrs, bSaveFrms, imgLims);
...
#             else: self.dfGroups = self.dfGroups.append(dfGroupsTmp);
    ...
    def getGrp_Attrs(self): #, dfFrame, dfGroupsFrm):
        ...
        for sMembers, dfGrp in self.dfGrp_Attrs.groupby(["GrpMembers"]):
            nInitFrm = dfGrp["Frame"][dfGrp.index[0]]; #InitFrm of this GrpMembers
            nLastIdx = dfGrp.index[-1];
#             print "nLastIdx:", nLastIdx;
            nPastFrm = float("nan"); #nInitFrm-1;
            for i,nFrm in enumerate(dfGrp["Frame"]): #Check Frms with this GrpMembers
                nIdx = dfGrp.index[i];
                nPastIdx = dfGrp.index[i-1] if i>0 else -1;
                #Set to InitFrm or EndFrm if this is First or Last iteration
                if i==0: #If First frm, then set InitFrm of current Sgmnt
                    dfGrp["InitFrm"][nIdx] = nFrm;
                if nLastIdx==nIdx: #If Last frm, then set EndFrm of current Sgmnt
                    dfGrp["EndFrm"][nIdx] = nFrm;
                #Check Consistency between frames (if past frm has same GrpMembers as current frm)
                if nFrm-1==nPastFrm: #If it's consistent with past frm, then just set InitFrm
                    dfGrp["InitFrm"][nIdx] = nInitFrm;
                elif nFrm-1!=nPastFrm and i>0: #Else, set to past frm the EndFrm                    
                    dfGrp["EndFrm"][nPastIdx] = nPastFrm;
                    #Reset the InitFrm into current frame
                    nInitFrm = dfGrp["Frame"][nIdx];
                    dfGrp["InitFrm"][nIdx] = nInitFrm;
                nPastFrm = nFrm;
            self.dfGrp_Attrs.update(dfGrp);
#         print self.dfGrp_Attrs[["GrpId", "Frame", "GrpMembers", "InitFrm", "EndFrm"]];
#                 nIdx = dfGrp.index[i];
#                 if nFrm-1==nPastFrm:
#                     dfGrp["InitFrm"][nIdx] = nInitFrm;
