...
def kSq_Exp(x,y): #Squared Exponential (aka Gaussian Kernel)
    v, l = 6, 5; #Variance & Characteristic Length Scale
...
if __name__ == '__main__':
    nSamples = 1;
    t = np.linspace(0.1,50,10).reshape(-1,1); #Queries x*
    x = np.array([np.linspace(0.1,50,10)[2], np.linspace(0.1,50,10)[8]]).reshape(-1,1);  #Data x
    y = np.array([-100,-100]).reshape(-1,1); #Data y [or f(x)]
    aData = np.array([x,y]);
    k = getKernel_PostPred(3, t, aData);
