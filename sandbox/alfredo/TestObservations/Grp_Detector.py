...
#         return dfGroups;
    ...
    def findClusters(self, dfFrame):
        aVX, aVY = np.asarray(dfFrame["SpeedX"])*50, np.asarray(dfFrame["SpeedY"])*50;
...
#         pl.hold(True);
        for label, grp, x, y in zip(aId, y_pred, aX, aY):
            if grp<0: text = str(label);
            else: text = str(label)+" Grp:"+str(grp);
...
#             aActors.append(self.aActors["TrackId"==int(actorId)]);
        newGrpActors = GrpActors(aActorsId, nGrpId);
