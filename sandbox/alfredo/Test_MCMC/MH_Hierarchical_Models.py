...
def f_TrgPdf(x): #Gaussian
    z = stats.multivariate_normal.pdf(x, mean=[3,-5], cov=np.array([[0.5,0],[0,5]]));
    return z;
def f_TrgPdf2(x): #Mixture of Gaussians
    z1 = stats.multivariate_normal.pdf(x, mean=[-2.5,6], cov=np.array([[1,0],[0,1]]));
    z2 = stats.multivariate_normal.pdf(x, mean=[2.5,11], cov=np.array([[1,0],[0,1]]));
    aW = [0.3, 0.7];
    return aW[0]*z1 + aW[1]*z2;
def f_TrgPdf3(x): #Beta-Bernoulli
...
def f_PropQ(x,c=None): #Multivariate Gaussian
    if c==None: #Sampler
        n = stats.multivariate_normal.rvs(x, np.eye(len(x)));
    else: #Proposal Distribution Pdf Q(x|c) [Mean=c]
        n = stats.multivariate_normal.pdf(x, mean=c, cov=np.eye(len(x)));
    return n;
def f_PropQ2(x,c=None): #Multivariate Gaussian
    if c==None: #Sampler
...
if __name__ == '__main__':
    nIters = 5000;
    aX = get_MH_HierModels(nIters, [0.7,0], f_TrgPdf3, f_PropQ2);
