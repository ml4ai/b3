...
def run_HDP():
    nG, nA, nC = 2, 2, 2; #Gamma, Alpha & Max No. Clusters
    aDir = [nG/nC]*nC;
    Dir0 = pm.Dirichlet('Dirichlet0', theta=aDir);  # @UndefinedVariable
    lDir0 = pm.Lambda('p_Dir0', lambda d=Dir0: np.concatenate([d,[1-sum(d)]])*nA);  # @UndefinedVariable
...
def get_DP(sDP, lDir0, aD):
    ...
    Dir = pm.Dirichlet('Dirichlet1_'+sDP, theta=lDir0);  # @UndefinedVariable
