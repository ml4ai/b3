...
#     print len(mNodes.keys());
    aRowCols = [int(np.ceil(len(mNodes)/nCols)), nCols];
    print len(mNodes), aRowCols;
    for i,k in enumerate(mNodes.keys()):
        ...
        bCategorical = type(mNodes[k])==pm.Categorical; #@UndefinedVariable
        nBins0 = nBins;
        if bBernoulli: nBins0 = 2;
        if bCategorical: nBins0 = len(set(node_Samples));
        if bDirichlet:
